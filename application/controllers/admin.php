...
/*
 * 
 *Admin controller for uoft theater project
 *
 */
...
Class Admin extends CI_Controller{
    ...
    function populate()
    {
	    ...
	    echo '<script language="javascript">';
  		echo 'alert("database populated")';  //not showing an alert box.
 		echo '</script>';
	    $data['main']='admin/index';
		$this->load->view('template', $data); 
...
function delete()
    {
	    ...
	    echo '<script language="javascript">';
  		echo 'alert("database deleted")';  //not showing an alert box.
 		echo '</script>'; 
		...
		function show_tickets(){
			...
			if ($tickets->num_rows() > 0){
				...
				$table[] = array('FirstName','LastName','Credit Card Number','Expiration Date','Showtime_id','Seat', 'Movie', 'Theater', 'Date', 'Time');
			
				
				foreach ($tickets->result() as $row){
					$table[] = array($row->first,$row->last,$row->creditcardnumber,$row->creditcardexpiration,$row->showtime_id,$row->seat,
							$row->title, $row->name, $row->date, $row->time);
				}
				$data['tickets'] = $table;
			}
			
			$data['main']='admin/show_tickets';
			$this->load->view('template', $data);
		...
		function delete_tickets(){
			...
			echo '<script language="javascript">';
  			echo 'alert("tickets deleted")';  //not showing an alert box.
 			echo '</script>';
	   		$data['main']='admin/index';
			$this->load->view('template', $data); 
		}
